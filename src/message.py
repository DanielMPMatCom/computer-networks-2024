# Based on https://en.wikipedia.org/wiki/List_of_FTP_server_return_codes
...
class error_not_expected(Error):
    pass


all_errors = (Error, OSError, EOFError)  # manage exceptions
...
# Each reply in the 4xx category might have a slightly different time value,
# but the intent is that the user-process is encouraged to try again.
...
# the User or Server (e.g., the command is spelled the same with the same arguments used; the user does
# not change his file access or user name; the server does not put up a new implementation.)

# 5xx	Permanent Negative Completion reply
# The command was not accepted and the requested action did not take place.
# The User-process is discouraged from repeating the exact request (in the same sequence).
# Even some "permanent" error conditions can be corrected, so the human user may want to direct his
# User-process to reinitiate the command sequence by direct action at some point in the future
# (e.g., after the spelling has been changed, or the user has altered his directory status.)
...
# 00 Series	The requested action is being initiated, expect another reply before proceeding with a new command.
